...
def _train(cfg, ar_model, device, logpath, train_loader, valid_loader, test_loader, loss_fn, multi_label, seed, lr, balanced, verbose):
    ...
    loss_fn = loss_fn if loss_fn is not None else (loss_bce if multi_label else loss_nll)
...
class TaskNetwork(torch.nn.Module):
    def __init__(self, cfg, ar):
        ...
        self.ar = ar
        print(cfg.feature_d, cfg.num_timestamps, cfg.runtime_cfg.hidden, cfg.runtime_cfg.n_class)
        # self.head = TaskHead(dim=cfg.feature_d, n_class=cfg.runtime_cfg.n_class, hidden=cfg.runtime_cfg.hidden)

        self.head = MSTGCN(num_of_timesteps=cfg.num_timestamps, num_of_vertices = cfg.num_of_vertices, num_of_features = cfg.num_of_features, 
                           k = cfg.cheb_k, num_of_chev_filters = cfg.num_of_chev_filters, num_of_time_filters = cfg.num_of_time_filters, 
                           time_conv_strides = cfg.time_conv_strides, time_conv_kernel = cfg.time_conv_kernel, num_block = cfg.num_block, 
                           dense_size = cfg.dense_size, GLalpha = cfg.GLalpha, dropout = cfg.dropout, cheb_polynomials=None)
    ...
    def forward(self, batch_wav):
        ...
        for i, pos in enumerate(position_names):
            ...
            if pos_mask.sum() > 0:  # At least some samples have this position
                ...
                pos_feat = self.ar(pos_audio)  # (batch_size, D, T)
                
                # Apply mask: zero out features for missing positions
                # Expand mask to match feature dimensions: (batch_size,) -> (batch_size, D, T)
                expanded_mask = pos_mask.unsqueeze(1).unsqueeze(2)  # (batch_size, 1, 1)
                pos_feat = pos_feat * expanded_mask  # Broadcast multiply
        ...
        # Stack all positions: (batch_size, 4, D, T)
        stacked_features = torch.stack(position_features, dim=1)
        
        # Transpose to MSTGCN input format: (batch_size, T, 4, D)
        # From: (batch_size, 4, D, T) -> (batch_size, T, 4, D)
...
def run_eval(config_file, task, options='', seed=42, lr=None, hidden=(), mixup=None, batch_size=None,
                          ...
                          optim='sgd', unit_sec=None, verbose=True, eval_only=False, freeze_ar=False, loss_fn=None, data_path='work'):
...
def finetune_main(config_file, task, options='', seed=42, lr=None, hidden=(), epochs=None, early_stop_epochs=None, warmup_epochs=None,
                  ...
                  optim='sgd', unit_sec=None, verbose=False, eval_only=False, freeze_ar=False, loss_fn=None, data_path='work'):
    scores, best_path, name, cfg, logpath = run_eval(config_file, task, options=options, seed=seed, lr=lr, hidden=hidden, mixup=mixup,
        ...
        unit_sec=unit_sec, verbose=verbose, eval_only=eval_only, freeze_ar=freeze_ar, loss_fn=loss_fn, data_path=data_path)
