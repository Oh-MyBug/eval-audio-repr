...
class MSTGCN(nn.Module):
    ...
    def __init__(self, num_of_timesteps, num_of_vertices, num_of_features, 
        ...
        self.dense_class = nn.Sequential(
            nn.Linear(num_of_time_filters*num_of_timesteps*num_of_vertices, dense_size),
            nn.Linear(dense_size, num_classes))
                
    def forward(self, x):
        x_GL, loss1, loss2 = self.block1(x)
        block_out = x_GL
        if self.num_block>1:
            for block in self.blocks:
                x_GL, loss1_tmp, loss2_tmp = block(block_out)
                block_out = x_GL
                loss1 += loss1_tmp
                loss2 += loss2_tmp
